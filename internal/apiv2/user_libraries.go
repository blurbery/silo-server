...
func registerUserLibraries(reg *Registry) {
	...
	Register(reg, operation("/user/libraries", "listUserLibraries"), func(ctx context.Context, _ *struct{}) (*UserLibraryListOutput, error) {
		...
		for _, view := range views {
			items = append(items, UserLibrary{ID: ID(strconv.Itoa(view.ID)), CertificationCountry: view.CertificationCountry, Name: view.Name, Type: view.Type, SortOrder: view.SortOrder, PosterURL: view.PosterURL})
		}
