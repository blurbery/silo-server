...
func (h *LibraryCollectionHandler) UploadAdminCollectionArtwork(ctx context.Context, id, kind string, data []byte) error {
	...
	return h.replaceCollectionImage(ctx, id, kind, data)
}
...
func (h *LibraryCollectionHandler) DeleteAdminCollectionArtwork(ctx context.Context, id, kind string) error {
	...
	if kind == collectionImagePoster {
		unlock, err := h.lockCollectionPosterMutation(ctx, id)
		if err != nil {
			return err
		}
		defer unlock()
	}
	if err := h.repo.Update(ctx, collectionImageRemovalUpdate(id, kind)); err != nil {
		return err
	}
	if err := h.deleteCollectionImages(ctx, id, kind); err != nil {
		slog.WarnContext(ctx, "collection image variants remain after database removal", "collection_id", id, "image_type", kind, "error", err)
	}
	return nil
}
