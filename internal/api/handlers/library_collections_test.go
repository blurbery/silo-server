...
func TestCollectionImageRemovalUpdate(t *testing.T) {
	poster := collectionImageRemovalUpdate("collection-1", "poster")
	if poster.ID != "collection-1" {
		t.Fatalf("poster removal ID = %q", poster.ID)
	}
	if poster.PosterURL == nil || *poster.PosterURL != "" {
		t.Fatalf("poster URL = %v, want pointer to empty string", poster.PosterURL)
	}
	if poster.PosterThumbhash == nil || *poster.PosterThumbhash != "" {
		t.Fatalf("poster thumbhash = %v, want pointer to empty string", poster.PosterThumbhash)
	}
	if poster.PosterAutoGenerated == nil || *poster.PosterAutoGenerated {
		t.Fatalf("poster auto-generated = %v, want false", poster.PosterAutoGenerated)
	}
	if poster.PosterSuppressed == nil || !*poster.PosterSuppressed {
		t.Fatalf("poster suppressed = %v, want true", poster.PosterSuppressed)
	}
	if poster.PosterFromTemplate == nil || *poster.PosterFromTemplate {
		t.Fatalf("poster from-template = %v, want false", poster.PosterFromTemplate)
	}

	backdrop := collectionImageRemovalUpdate("collection-2", "backdrop")
	if backdrop.BackdropURL == nil || *backdrop.BackdropURL != "" {
		t.Fatalf("backdrop URL = %v, want pointer to empty string", backdrop.BackdropURL)
	}
	if backdrop.BackdropThumbhash == nil || *backdrop.BackdropThumbhash != "" {
		t.Fatalf("backdrop thumbhash = %v, want pointer to empty string", backdrop.BackdropThumbhash)
	}
	if backdrop.PosterSuppressed != nil {
		t.Fatalf("backdrop removal changed poster suppression: %v", *backdrop.PosterSuppressed)
	}
}

func TestApplyCollectionPosterURLUpdate(t *testing.T) {
	tests := []struct {
		name           string
		posterURL      string
		wantSuppressed bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if input.PosterAutoGenerated == nil || *input.PosterAutoGenerated {
				t.Fatalf("poster auto-generated = %v, want false", input.PosterAutoGenerated)
			}
			if input.PosterFromTemplate == nil || *input.PosterFromTemplate {
				t.Fatalf("poster from-template = %v, want false", input.PosterFromTemplate)
			}
			if input.PosterThumbhash == nil || *input.PosterThumbhash != "" {
				t.Fatalf("poster thumbhash = %v, want empty", input.PosterThumbhash)
			}
		})
	}
}

func TestCollectionAllowsRequestedPosterWrite(t *testing.T) {
	tests := []struct {
		name       string
		...
		{name: "missing collection", requested: "/poster.svg"},
		{name: "empty request", collection: &models.LibraryCollection{}},
		{name: "suppressed", collection: &models.LibraryCollection{PosterSuppressed: true}, requested: "/poster.svg"},
		{name: "empty current poster", collection: &models.LibraryCollection{}, requested: "/poster.svg", want: true},
		{name: "auto-generated current poster", collection: &models.LibraryCollection{PosterURL: "collection-images/old.webp", PosterAutoGenerated: true}, requested: "/poster.svg", want: true},
		{name: "template current poster", collection: &models.LibraryCollection{PosterURL: "collection-images/old.webp", PosterFromTemplate: true}, requested: "/poster.svg", want: true},
		{name: "just-created matching placeholder", collection: &models.LibraryCollection{PosterURL: "/poster.svg"}, requested: " /poster.svg ", want: true},
		{name: "different sticky custom poster", collection: &models.LibraryCollection{PosterURL: "collection-images/custom.webp"}, requested: "/poster.svg"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := collectionAllowsRequestedPosterWrite(tt.collection, tt.requested); got != tt.want {
				t.Fatalf("allowed = %t, want %t", got, tt.want)
			}
		})
	}
}
